...
for sub_work_dir in work_dirs:
        ...
        with open(work_dir, "r", encoding="utf-8") as reader:
            ...
            for line in lines:
                ...
                if 'Eval' in line:
                    ...
                    valid_step.append(float(filter_split[4]))
                    valid_loss.append(float(filter_split[9]))
                    valid_ppl.append(float(filter_split[12]))
                    continue
                if 'Exiting' in line:
                    continue
                if 'End' in line:
    ...
    #plt.plot(np.array(valid_step),np.array(valid_loss),c=colo,label=label)
    plt.plot(np.array(valid_step[25:]),np.array(valid_loss[25:]),c=colo,label=label)
plt.title('Validation Loss of T-XL models') 
plt.ylabel('Loss')
plt.xlabel('Steps')   
plt.legend()
plt.show()
...
plt.savefig('valid_loss_midway_plots_txl.png',dpi=400)
